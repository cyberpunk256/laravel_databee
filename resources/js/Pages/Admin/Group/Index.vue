...
<template>
  <AdminLayout>
    <div class="mb-5">
      <h5 class="text-h5 font-weight-bold">グループ一覧</h5>
    </div>
    <v-card class="pa-4">
      <div class="d-flex flex-wrap align-center">
        ...
        />
        <v-spacer />
        <Link href="/admin/group/create" as="div">
      ...
      >
        <template #[`item.action`]="{ item }">
          <Link :href="`/admin/group/${item.value}/edit`" as="button">
...
export default {
  ...
  methods: {
    loadItems({ page, itemsPerPage, sortBy, search }) {
      ...
      this.$inertia.get('/admin/group', params, {
        preserveState: true,
        preserveScroll: true,
        onSuccess: () => {
    ...
    submitDelete() {
      this.isLoading = true
      this.$inertia.delete(`/admin/group/${this.deleteId}`, {
        preserveState: true,
        preserveScroll: true,
        onSuccess: () => {
